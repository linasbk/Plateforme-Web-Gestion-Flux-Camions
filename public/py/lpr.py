...
def lpr(image, show_steps=False, show_contour=True, show_plates=True, show_hist=True):
    ...
    text = pytesseract.image_to_string(
        Image.fromarray(clearplate), config='', lang="ara")
...
def separate_resize_plate(image, out, apr, cnt, show_hist=False):
    ...
    for i in range(w):
        for j in range(h):
            if mask[i][j] == 0:
                clone[i][j] = np.int8(
                    (255 / pixels) * cumulative[cleanimg[i][j]])
...
def calculate_otsu(hist):  # adapted from wikipedia
    ...
    for t in range(nbins):
        ...
        miu_L = sum(np.dot(p[:t], np.transpose(
            np.matrix([i for i in range(t)])))) / q_L
        miu_H = sum(np.dot(p[t:], np.transpose(
            np.matrix([i for i in range(t, 256)])))) / q_H
...
def find_license_plate(image, accepted_ratio=3.72, error=3.1):

    area_threshold = 1800  # arbitrary threshold for plate area in image.
